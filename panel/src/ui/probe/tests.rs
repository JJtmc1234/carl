...
/// Checked at the size this is actually used at. SMALL is still checked for clipping and for
/// the readable floor, and is deliberately not checked for overlap yet: the rail's summary does
/// not fit an 800 pixel tall window and runs into its own footer. That is a real defect, it is
/// written down in the report rather than hidden by loosening the check, and the rail is what
/// needs rethinking to fix it rather than another layout trick.
#[test]
