...
pub fn sized_card(ui: &mut Ui, rect: Rect, spec: Card, add: impl FnOnce(&mut Ui)) -> Response {
    ...
    // Intersected with whatever is already clipping us, never set outright. `set_clip_rect`
    // replaces the clip rather than narrowing it, so a card sitting half below the fold of a
    // scroll area would otherwise hand its contents a clip reaching past that area and paint
    // them over whatever is underneath. That is how the overview columns ended up drawing on
    // top of the workspace header at 1280x800.
