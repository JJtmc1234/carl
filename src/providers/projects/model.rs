...
// `ProjectId` used to live here. It moved to the crate root so `army::task` can name a project
// without `army` depending on this layer, which would be backwards: a task is a lower thing than
// a project provider. Re-exported rather than replaced, so every existing use still reads the
// same and there is still exactly one of it.
pub use crate::ProjectId;
